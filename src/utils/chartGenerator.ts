...
export async function generatePriceChart(history: PriceHistory[], width = 400, height = 200): Promise<Uint8Array> {
    ...
    <text x="${padding.left - 5}" y="${height - padding.bottom}" text-anchor="end" fill="#64748b" font-size="10">${minPrice.toFixed(
		2
	)}</text>

    <!-- X-axis labels -->
    <text x="${padding.left}" y="${height - padding.bottom + 20}" text-anchor="middle" fill="#64748b" font-size="10">${new Date(
		sortedHistory[0].recordedAt
	).toLocaleDateString()}</text>
    <text x="${width - padding.right}" y="${height - padding.bottom + 20}" text-anchor="middle" fill="#64748b" font-size="10">${new Date(
